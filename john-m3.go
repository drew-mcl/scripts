...
// It now includes the new `depends_on_all_of` field.
package topology
...
func TestInferAndValidateShardCounts(t *testing.T) {
    testCases := []struct {
        ...
        {
            name: "Error on unused shard definition",
            rawTopo: YAMLTopology{
                Shards: map[string]int{"c": 2},
                Apps:   map[string]AppDefinition{"a": {}, "b": {}},
            },
            groups:      map[string][]string{"a": {"a", "b"}},
            expectErr:   true,
            errContains: "shard count defined for non-existent app 'c'",
        },
    }
...
func TestLinkDependencies(t *testing.T) {
	// A simple graph setup for testing dependency linking
	graph := &Graph{Nodes: make(map[string]*Node)}
	graph.Nodes["bog-00"] = &Node{ID: "bog-00", BaseApp: "bog"}
	graph.Nodes["bog-01"] = &Node{ID: "bog-01", BaseApp: "bog"}
	graph.Nodes["sor-00"] = &Node{ID: "sor-00", BaseApp: "sor"}
	graph.Nodes["sor-01"] = &Node{ID: "sor-01", BaseApp: "sor"}
	graph.Nodes["db"] = &Node{ID: "db", BaseApp: "db"}

	rawTopo := YAMLTopology{
		Apps: map[string]AppDefinition{
			"bog": {DependsOnAllOf: []string{"sor"}},
			"sor": {DependsOn: []string{"db"}},
			"db":  {},
		},
	}
	appShardCounts := map[string]int{"bog": 2, "sor": 2, "db": 1}

	err := linkDependencies(graph, rawTopo, appShardCounts)
	if err != nil {
		t.Fatalf("linkDependencies failed: %v", err)
	}

	// Check fan-in dependency for bog-00
	bog00Deps := graph.Nodes["bog-00"].DependsOn
	if len(bog00Deps) != 2 {
		t.Errorf("expected bog-00 to have 2 dependencies, got %d", len(bog00Deps))
	}
	depIDs := map[string]bool{}
	for _, dep := range bog00Deps {
		depIDs[dep.ID] = true
	}
	if !depIDs["sor-00"] || !depIDs["sor-01"] {
		t.Error("bog-00 is missing correct fan-in dependencies to sor shards")
	}

	// Check standard dependency for sor-00
	sor00Deps := graph.Nodes["sor-00"].DependsOn
	if len(sor00Deps) != 1 || sor00Deps[0].ID != "db" {
		t.Error("sor-00 should depend on db")
	}
}
